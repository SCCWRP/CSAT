#' Update CSAT packages
#'
#' This will check to see if all CSAT packages (and optionally, their
...
#' CSAT_update()
#' }
CSAT_update <- function(recursive = FALSE) {
  
  deps <- CSAT_deps(recursive)
  behind <- dplyr::filter(deps, behind)
  
  if (nrow(behind) == 0) {
    cli::cat_line("All CSAT packages up-to-date")
    return(invisible())
  }
...
#' List all CSAT dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   CSAT packages.
#' @export
CSAT_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("CSAT", pkgs, recursive = recursive)
