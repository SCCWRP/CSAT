#' Conflicts between CSAT and other packages
#'
#' This function lists all the conflicts between packages in CSAT
...
#' @examples
#' CSAT_conflicts()
CSAT_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))
  
  conflicts <- purrr::keep(objs, ~ length(.x) > 1)
  
  tidy_names <- paste0("package:", CSAT_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "CSAT_conflicts")
}

CSAT_conflict_message <- function(x) {
  if (length(x) == 0) return("")
  
  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "CSAT_conflicts()"
  )
...
#' @export
print.CSAT_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(CSAT_conflict_message(x))
}
